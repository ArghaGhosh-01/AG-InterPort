...
const InteractiveBoxesSection = () => {
  ...
  const MAX_ROTATION = 10; // Reduced max rotation to prevent overflow

  // Responsive setup with extra padding for rotation
  useEffect(() => {
    const updateDimensions = () => {
      const isMobileDevice = window.innerWidth <= 768;
      setIsMobile(isMobileDevice);
      
      // Add extra height to accommodate rotated boxes
      const baseHeight = Math.max(window.innerHeight / (isMobileDevice ? 3.5 : 3), 250);
      const rotationPadding = 80; // Extra space for rotation
      const vh = baseHeight + rotationPadding;
  ...
  useEffect(() => {
    ...
    const rotationBuffer = 40; // Buffer for rotation space
    ...
    // Random position function with collision avoidance and rotation buffer
    const getRandomPosition = (box, placedBoxes, maxAttempts = 100) => {
      ...
      while (attempts < maxAttempts) {
        ...
        // Random position within container bounds with rotation buffer
        const x = Math.random() * (containerWidth - box.width - rotationBuffer * 2) + rotationBuffer;
        const y = Math.random() * (containerHeight - box.height - rotationBuffer * 2) + rotationBuffer;
      ...
      // If we couldn't find a non-overlapping position, use buffer zone
      return {
        x: Math.random() * (containerWidth - box.width - rotationBuffer * 2) + rotationBuffer,
        y: Math.random() * (containerHeight - box.height - rotationBuffer * 2) + rotationBuffer
      };
    };
    ...
    allBoxes.forEach(otherBox => {
      if (checkCollision(box, otherBox)) {
        ...
        if (overlapX < overlapY) {
          ...
          box.angularVelocity += boxCollisionY * box.vx * 0.005; // Reduced rotation effect
          otherBox.angularVelocity -= otherBoxCollisionY * otherBox.vx * 0.005;
          ...
          box.angularVelocity -= boxCollisionX * box.vy * 0.005; // Reduced rotation effect
          otherBox.angularVelocity += otherBoxCollisionX * otherBox.vy * 0.005;
  ...
  // Enhanced boundary enforcement with rotation buffer
  const enforceBoundaries = (box, containerWidth, containerHeight) => {
    let { x, y, width, height, vx, vy, ax, ay, rotation, angularVelocity } = box;
    const rotationBuffer = 40; // Buffer space for rotation
    
    if (x < rotationBuffer) {
      x = rotationBuffer;
      vx = -vx * BOUNCE;
      ax = -ax * 0.5;
      angularVelocity += vy * 0.1; // Reduced rotation effect
    } else if (x + width > containerWidth - rotationBuffer) {
      x = containerWidth - width - rotationBuffer;
      vx = -vx * BOUNCE;
      ax = -ax * 0.5;
      angularVelocity -= vy * 0.1;
    }
    ...
    if (y < rotationBuffer) {
      ...
      vy = -vy * BOUNCE;
      ay = -ay * 0.5;
      angularVelocity -= vx * 0.05; // Reduced rotation effect
    } else if (y + height > containerHeight - rotationBuffer) {
      y = containerHeight - height - rotationBuffer;
      vy = -vy * BOUNCE;
      ay = -ay * 0.5;
      angularVelocity += vx * 0.05;
    }
    ...
    // Limit maximum rotation more strictly
    if (Math.abs(rotation) > MAX_ROTATION) {
      rotation = MAX_ROTATION * Math.sign(rotation);
      angularVelocity *= 0.3; // More aggressive slowdown
    }
  ...
  // Drag handling with reduced rotation effects
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    const handleMouseMove = (e) => {
      ...
      const mouseX = e.clientX - rect.left;
      const mouseY = e.clientY - rect.top;
      const rotationBuffer = 40;

      let newX = mouseX - draggedBox.offsetX;
      let newY = mouseY - draggedBox.offsetY;
      
      // Enforce boundaries with rotation buffer
      newX = Math.max(rotationBuffer, Math.min(newX, rect.width - draggedBox.width - rotationBuffer));
      newY = Math.max(rotationBuffer, Math.min(newY, rect.height - draggedBox.height - rotationBuffer));
      
      const vx = (newX - draggedBox.x) * 0.5;
      const vy = (newY - draggedBox.y) * 0.5;
      
      // Reduced rotation effect during drag
      let newRotation = draggedBox.rotation;
      if (Math.abs(vx) > 1) {
        newRotation = Math.max(-MAX_ROTATION, Math.min(MAX_ROTATION, vy * 0.2)); // Reduced multiplier
      }
    ...
    const handleMouseUp = () => {
      if (isDragging && draggedBox) {
        ...
        const angularVelocity = (dx + dy) * 0.05; // Reduced angular velocity
    ...
    const handleTouchMove = (e) => {
      ...
      let newRotation = draggedBox.rotation;
      if (Math.abs(vx) > 1) {
        newRotation = Math.max(-MAX_ROTATION, Math.min(MAX_ROTATION, vy * 0.2));
      }
...
const Container = styled.div`
  ...
  margin: 0 15px;
  user-select: none;
  /* Add extra padding to prevent visual overflow */
  padding: 20px 0;
